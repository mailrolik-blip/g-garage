(() => {
  ...
  function managerFlow(route) {
    ...
    let body;
    if (route === "manager-dashboard") body = `<div class="module-grid manager-metrics">${cards.map(([to,label,count]) => `<button class="card module-card" data-go="${to}"><b>${label}</b><div class="metric">${count}</div></button>`).join("")}</div><div class="manager-columns"><section class="card pad"><h2>SLA сегодня</h2>${vinRequests.map(vinCard).join("")}</section><section class="card pad"><h2>Заказы в работе</h2>${orders.slice(0,3).map(orderCard).join("")}</section></div>`;
    else if (route.includes("vin")) body = `<div class="manager-columns"><section class="card pad"><div class="row between"><h2>Очередь VIN</h2><span class="status warn">SLA 42 мин</span></div>${vinRequests.map(vinCard).join("")}</section><section class="card pad manager-detail"><h2>${routeMap.get(route).title}</h2><p><b>Клиент:</b> тестовый контакт · <b>Менеджер:</b> Антон</p><p><b>Авто:</b> ${selectedCar().brand} ${selectedCar().model} ${selectedCar().year}</p><div class="skeleton manager-chat"></div><div class="grid two"><button class="btn secondary" data-go="/manager-request-data">Запросить данные</button><button class="btn" data-go="/manager-product-search">Искать товары</button><button class="btn secondary" data-go="/manager-preview">Предпросмотр</button><button class="btn" data-go="/manager-send">Отправить</button></div></section></div>`;
    else if (route.includes("order")) body = `<div class="manager-columns"><section class="card pad"><h2>Заказы</h2>${orders.map(orderCard).join("")}</section><section class="card pad"><h2>${routeMap.get(route).title}</h2><p>Статус, клиент, доставка и документы доступны в одном рабочем экране.</p>${button("/manager-order-status", "Изменить статус", "full")}${button("/manager-contact", "Связаться с клиентом", "secondary full")}</section></div>`;
    else if (route.includes("product") || route === "manager-catalog") body = `<div class="manager-columns"><section class="card pad"><h2>Поиск товаров</h2><div class="search-box">${icon.search}<input placeholder="Артикул или название"></div><div class="grid manager-product-grid">${products.slice(0, 8).map(productCard).join("")}</div></section><aside class="card pad sticky-summary"><h2>Подборка</h2>${compareTable()}${button("/manager-preview", "Предпросмотр клиенту", "full")}</aside></div>`;
    else body = `<div class="card pad"><p>Операционный экран: менеджер видит контекст клиента, статусы, подборку и следующий шаг.</p>${button("/manager-dashboard", "К дашборду", "full")}</div>`;
    return `<div class="manager-workspace"><aside class="manager-sidebar desktop-only"><b>G-Garage Ops</b>${["/manager-dashboard:Дашборд","/manager-vin-queue:VIN-очередь","/manager-orders:Заказы","/manager-product-search:Товары","/manager-promos:Акции","/manager-settings:Настройки"].map((item) => { const [to,label]=item.split(":"); return `<button data-go="${to}">${label}</button>`; }).join("")}</aside><section><div class="manager-topbar desktop-only"><div><b>${routeMap.get(route)?.title || "Менеджер"}</b><p class="small muted">SLA, очередь, клиент, подборка и коммуникации</p></div><button class="btn secondary" data-go="/manager-vin-queue">Новая VIN</button></div>${title(routeMap.get(route)?.title || "Менеджер")}${body}</section></div>`;
  }
